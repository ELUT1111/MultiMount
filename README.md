...
首次启动时会自动创建超级管理员账号 `admin`，密码按以下规则生成:

- 如果设置了环境变量 `ADMIN_PASSWORD`，则使用该值作为初始密码
- 否则随机生成一个安全密码，并以 WARNING 级别输出到启动日志（控制台及 `backend/logs/app.log`），形如:

```
管理员账号已创建 — 用户名: admin, 密码: xxxxxxxxxxxxxxxx
```

> 注意: 密码只在**首次创建**时生成并打印一次，之后重启不会重新生成或再次显示。如果忘记密码，可删除数据库文件后设置 `ADMIN_PASSWORD` 重新启动。
